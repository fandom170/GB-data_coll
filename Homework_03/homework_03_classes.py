...
class PageProcessing():
    ...
    def get_next_page_link(self, s_page):
        ...
        if self.main_link == 'https://hh.ru':
            try:
                next_page_link = s_page.find('a', {'class': 'bloko-button', 'data-qa': 'pager-next'})['href']
                return next_page_link
            except TypeError:
                return None
        elif self.main_link == 'https://www.superjob.ru':
            try:
                next_page_link = s_page.find('a', {'class': 'f-test-link-Dalshe'})['href']
                return next_page_link
            except TypeError:
                return None
        else:
    ...
    def salary_processing_hh(self, job_salary_block):
        ...
        if len(job_salary) == 4:
            min_salary = float(job_salary[0])
            max_salary = float(job_salary[2])
        elif job_salary[0].rstrip() == 'от':
            min_salary = float(job_salary[1])
            max_salary = None
        elif job_salary[0].rstrip() == 'до':
            min_salary = None
            max_salary = float(job_salary[1])
        else:
            return None, None, None
    ...
    def write_results(self, filename, all_jobs):
        ...
        result = json.dumps(all_jobs, indent=4, ensure_ascii=False, cls=JSONEncoder)
        file = filename + '.json'
